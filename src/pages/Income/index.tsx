...
import { TrendingUp, Plus, Pencil, Ban } from 'lucide-react'
...
export function IncomePage() {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Ingresos"
        description="Registro y seguimiento de ingresos"
        actions={
          <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Total ingresos"  value={formatCurrency(total)}   icon={TrendingUp} trend="up" />
        <StatCard label="Registros"       value={String(txs.length)}      icon={TrendingUp} />
        <StatCard label="Completados"     value={String(completed)}       icon={TrendingUp} trend="up" />
        <StatCard label="Pendiente cobrar" value={formatCurrency(pending)} icon={TrendingUp} trend="neutral" />
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-base-border">
              ...
              {!isLoading && txs.length === 0 && (
                <tr><td colSpan={7} className="py-16 text-center text-content-muted text-sm">No hay ingresos en este período</td></tr>
              )}
              {txs.map((tx) => (
                <tr key={tx.id} className="hover:bg-base-elevated/50 transition-colors">
                  ...
                  <td className={cn('px-4 py-3 font-semibold tabular-nums text-emerald-400', tx.status === 'CANCELLED' && 'line-through opacity-50')}>
