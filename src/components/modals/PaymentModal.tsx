...
import type { ApiReceivable } from '@/types/api'
...
export function PaymentModal({ open, onClose, receivable }: PaymentModalProps) {
  ...
  const pending = receivable
    ? Number(receivable.amount) - Number(receivable.amountPaid)
    : 0
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className={cn(
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-base-border">
            <Dialog.Title className="text-sm font-semibold text-content-primary">Registrar pago</Dialog.Title>
          ...
          <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
            ...
            <div className="rounded-lg bg-base-elevated border border-base-border p-3 space-y-1">
              <p className="text-xs text-content-muted">Factura</p>
              <p className="text-sm font-medium text-content-primary">{receivable.description}</p>
              <p className="text-xs text-content-muted">{receivable.client.name}</p>
              <div className="flex justify-between pt-1">
                <span className="text-xs text-content-muted">Total: {formatCurrency(Number(receivable.amount))}</span>
