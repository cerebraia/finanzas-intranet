...
export function FixedExpenseModal({ open, onClose }: FixedExpenseModalProps) {
  ...
  useEffect(() => {
    if (open) {
      setName(''); setAmount(''); setCategoryId(''); setPaymentDay(''); setAccountId('')
      setStartDate(new Date().toISOString().slice(0, 7) + '-01'); setNotes('')
    }
  }, [open])
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className={cn(
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-base-border">
            <Dialog.Title className="text-sm font-semibold text-content-primary">Nuevo gasto fijo</Dialog.Title>
          ...
          <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button type="submit" disabled={createExpense.isPending} className="flex-1 py-2 rounded-lg bg-brand-600 hover:bg-brand-500 text-white text-sm font-semibold transition-all disabled:opacity-50">
                {createExpense.isPending ? 'Guardando...' : 'Guardar gasto fijo'}
              </button>
            </div>
          </form>
        </Dialog.Content>
